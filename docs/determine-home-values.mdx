...
### Understanding the yearBuilt filter

The `yearBuilt` field in the queries above (e.g., `yearBuilt:>=2018 AND yearBuilt:<=2022`) directly tells you the year the physical structure was originally constructed. This is how you can pinpoint exactly when a property was built — particularly useful for buildings and commercial residentials, rather than apartments where construction dates may be less precise or meaningful for comparison purposes.

By filtering on `yearBuilt`, you ensure that comparable properties share a similar age and construction era, which is critical for accurate home valuations. A home built in 2020 will have different materials, code compliance, and depreciation characteristics than one built in 1980, all of which affect market value.
...
<Callout kind="info" collapsed="false">
  **Using credits**
  ...
  Your "num\_records" sets your limit on how many credit to use. If your query is under the limit, Datafiniti will only charge property for the lower value. For more info on how to spend credits, you can read [How Credits Work](/docs/how-credits-work-api)
</Callout>
