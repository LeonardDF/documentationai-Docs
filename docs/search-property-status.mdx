...
Datafiniti's property data can be used to verify or enrich data that you have for existing properties. If you have an area of property but want more specific data about its status on the market, you can use the following guide as a starting point.
...
Let's say you are looking for a single-family dwelling for sale in Sacramento, California.\
...
<Callout kind="tip" collapsed="false">
  **Fuzzy Matching**
  ...
  The `address` field defaults to fuzzy matching for exact, full, or partial addresses. This means the following queries would all return matching results:

  - `address:\"2815 Manor Rd\"`
  - `address:\"2815 Manor Road\"`
  - `address:\"2815 Manor Rd, Austin\"`

  For more details on how fuzzy matching and address normalization work, including address prerequisites, supported formats, ambiguous state/suffix codes, and other behavior, see [Constructing Property Queries — Fuzzy Matching](/docs/constructing-property-queries#fuzzy-matching).
</Callout>
