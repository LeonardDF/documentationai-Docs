...
For the full parameter definition, see the [Property Data API reference](https://docs.datafiniti.co/reference/properties).
...
Add the `fuzzy_match` parameter alongside your normal search parameters. It accepts one of three values:

- `query` - This selects which records should be matched. It's the same format as our search endpoint.
- `fuzzy_match` - Controls how the `address` field is matched against Datafiniti's records. Set to `on`, `off`, or `retry`.
- `num_records` - The number of records matching the query you want returned. If this parameter is not specified, all matching records will be returned.
...
Check out some of our use cases where `fuzzy_match` is helpful:
