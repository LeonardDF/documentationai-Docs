...
Product records include a `categories` field that contains category keywords collected from multiple sources. The `categories` field is defined in the [Product Data Schema](/docs/product-data-schema). For standardized top-level and second-level categories, see the [`primaryCategories`](#standardized-categories)[ and ](#standardized-categories)[`secondaryCategories`](#standardized-categories) fields.
