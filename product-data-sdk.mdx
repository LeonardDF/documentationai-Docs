...
<CodeGroup default-tab="1" show-lines="true" tabs={["pip"]}>
  ```bash pip
...
<CodeGroup show-lines="true" tabs={["product_search.py","product_count.py","product_full_query.py","product_paginate.py"]}>
  ```python product_search.py
  from datafiniti.product import DatafinitiProductSDK
  from datafiniti.errors import DatafinitiAPIError

  # ---------------------------------------------------------------------------
  # Product lookups — swap SEARCH_MODE to try different query strategies.
  #
  #   "by_name"   Product name, optionally narrowed by brand.
  #               Use when you have a product title and want matching records.
  #
  #   "by_gtin"   Exact GTIN (UPC, EAN, ISBN) match.
  #               Use when you have a barcode and want the matching product.
  #
  #   "by_brand_mpn"  Brand + manufacturer part number.
  #               Use to pinpoint a specific SKU within a brand's catalog.
  #
  # Reference: https://docs.datafiniti.co/docs/searching-by-gtin
  # ---------------------------------------------------------------------------
  ...
  if SEARCH_MODE == "by_name":
      query = sdk.query().name("iPhone 15").brand("Apple").build()
  elif SEARCH_MODE == "by_gtin":
      query = 'gtins:"885909950805"'
  elif SEARCH_MODE == "by_brand_mpn":
      query = 'brand:"Apple" AND mpn:"MTP63LL/A"'
  else:
  ...
  try:
      ...
      for product in response.get("records", []):
          ...
          print(f"GTIN: {gtins[0] if gtins else 'N/A'}")
          if prices:
              p = prices[0]
              print(f"Price: {p.get('amountMin', 'N/A')} {p.get('currency', '')}")
          print()
  ...
  VIEW = [
      ...
      {"flatten": False, "sub_fields": [], "name": "brand"},
      {"flatten": False, "sub_fields": [], "name": "gtins"},
      {"flatten": False, "sub_fields": [], "name": "primaryCategories"},
      {"flatten": True, "sub_fields": ["amountMin", "amountMax", "currency"], "name": "prices"},
  ]
  ...
  try:
      ...
      response = sdk.search(query=query, num_records=5, view=VIEW)
  ...
  try:
      for product in sdk.paginate(
          query='brand:"Apple" AND categories:"Cell Phones"',
          # note that the query can be modified to any product data query
...
## <a href="https://docs.datafiniti.co/people-data-sdk#code-examples" target="_blank" rel="noopener noreferrer">**Code Examples**</a>

Here are some product data Python examples built upon what we have just discussed. All scripts here should be ready to run as long as you have your `DATAFINITI_API_KEY `exported`.`

<CodeGroup default-tab="0" show-lines="true" tabs={["Product_Count.py","Product_Categories.py","Product_Paginate.py","Product_Pricelist.py","Product_Views.py"]}>
  ```python Product_Count.py
  from datafiniti.product import DatafinitiProductSDK
  from datafiniti.errors import DatafinitiAPIError
